...
void Key_Control_LED(GPIO_TypeDef* LED_Port, uint16_t LED_Pin,
                        GPIO_TypeDef* KEY_Port, uint16_t KEY_Pin,
                        uint8_t mode, uint8_t *last_state, uint32_t *tick)
{
    ...
    else    //ģʽ�趨�����·�ת
    {
        ...
        if(*last_state == GPIO_PIN_SET && now_state == GPIO_PIN_RESET 
           && (HAL_GetTick() - *tick) > 20)
        {
